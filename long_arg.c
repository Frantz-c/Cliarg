...
static int		get_arg_long_bool(t_cli *cli, int ac, char **av, int checked[])
{
	int			i;
...
static int		get_arg_long(t_cli *cli, int ac, char **av, int checked[])
{
	char		*eq_pos;
...
// next arg
static int		get_arg_long_next(t_cli *cli, int ac, char **av, int checked[])
{
	int		i;
...
static int		get_arg_long_next_bool(t_cli *cli, int ac, char **av, int checked[])
{
	int		i;
...
extern int     search_long_argument(t_cli *arg, int ac, char *av[], int checked[])
{
    ...
    if (arg->value_is_next_arg)
    {
        if (arg->type == BOOL_TYPE)
            error = get_arg_long_next_bool(arg, ac, av, checked);
        else
            error = get_arg_long_next(arg, ac, av, checked);
    }
    else
    {
        if (arg->type == BOOL_TYPE)
            error = get_arg_long_bool(arg, ac, av, checked);
        else
            error = get_arg_long(arg, ac, av, checked);
    }
    return (error);
}
