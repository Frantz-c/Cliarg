...
t_cli	**cli_search_arguments(t_cli **arg, int ac, char **av)
{
	...
	if (i > 0) // pas assez d'arguments
	{
		...
		while (j < i)
		{
			...
			arg[j]->value.str = ft_strdup(av[j]);
			avstat.checked[j] = 1;
			avstat.remaind--;
	...
	if (arg[i])
	{
		...
		while (nlast)
		{
			...
			{
				arg[i]->value.str = ft_strdup(av[j]);
				avstat.checked[j] = 1;
				avstat.remaind--;
	...
	while (arg[i])
	{
		if (arg[i]->sname && arg[i]->stat.valnextarg && !arg[i]->stat.traited)
		{
...
int				duplicate_argument(int ac, char **av)
{
	...
	while (ac)
	{
		...
		while (i)
		{
			...
			{
				set_cli_err(CLI_DUPLICATE_AV, av[i]);
				return (CLI_ERROR);
			}
		}
	}
...
extern t_cli	**get_args_from_cli(const char **fmt, int ac, char **av)
{
	...
	while (fmt[i])
	{
		argument = fmt[i];
		arg[i] = new_arg();
		arg[i]->stat.valnextarg = value_is_next(&argument);
